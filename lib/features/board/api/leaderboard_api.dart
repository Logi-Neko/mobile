...
class LeaderboardApi {
  /// Lấy bảng xếp hạng toàn cầu (sử dụng API có sẵn)
  /// Sử dụng getUserDashboard từ HomeApi - đã trả về List<AccountShowResponse>
  static Future<ApiResponse<List<AccountShowResponse>>> getGlobalLeaderboard() async {
    try {
      return await HomeApi.getUserDashboard();
    } catch (e) {
      rethrow;
    }
  }

  /// Lấy danh sách bạn bè (sử dụng API có sẵn)
  /// Sử dụng getFriendsList từ FriendshipApi
